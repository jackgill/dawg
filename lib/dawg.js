...
function serve(source, options) {
    ...
    if (options.silent) logger.silent = true;
    ...
    function updateChapters() {
        logger.log('serv', 'Gathering chapters from ' + source);
    ...
    function renderChapters() {
        ...
        logger.log('serv', 'Rerendering ' + chapters.length + ' chapters');
    ...
    function handleRequest(request, response) {
        ...
        if (!chapter) {
            logger.log('http', ' >> 404 - ' + request.url);
            response.writeHead(404);
            response.write('404 - Not Found');
        }
        else {
            logger.log('http', ' >> 200 - ' + request.url);
    ...
    logger.log('serv', 'dawg listening on ' + options.host + ':' + options.port);
}
...
function convert(source, destination, options) {
    ...
    function converter() {
        logger.log('conv', 'Converting ' + source + ' to ' + destination);

        // Clean the destination for the chapter(s)
        cleanDestination(destination, options.clean);

        // Find all chapters in the source directory
        logger.log('conv', 'Gathering chapters from ' + source);
 ...
 * Returns nothing.
 */
function cleanDestination(destination, unlink) {
    logger.log('conv', 'Cleaning destination ' + destination);
    ...
    var ext = path.extname(destination).length > 0;

    if ((stat.isDirectory() || !ext) && unlink) {
...
function write(chapters, destination, options) {
    ...
    logger.log('conv', 'Rendering ' + chapters.length + ' chapters');
    ...
    logger.log('conv', 'Writing ' + chapters.length + ' chapters to ' + destination);
