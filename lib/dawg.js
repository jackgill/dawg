...
function options(options) {
    ...
    if (_.has(options, 'host') && options.host === 'localhost') {
...
function write(chapters, destination, options) {
    ...
    var type;
    try {
        type = prepareDestination(destination, options.clear);
    }
    catch(error) {
        log.error('Could not write ' + chapters.length + ' chapters to ' + destination);
        return;
    }
    ...
    _.each(rendered, function(content, id) {
        ...
        // Base the filename on the original filename, or the destination if it should
        // be a file
        var target;
        if (type.isFile) {
            target = destination;
            ...
            target = path.join(destination, utils.stripExtension(chapter.filename) + '.html');
        }
...
/**
 * Internal: Prepare a destination path. If the path appears to be a directory
 ...
 * Returns nothing.
 */
function prepareDestination(destination, unlink) {
    ...
    var shouldBeFile = (wasFile || (!wasDirectory && path.extname(destination).length > 0));
    ...
    if ((!wasDirectory && !shouldBeFile) || (wasDirectory && unlink)) {
        // Recreate the destination if it was or should be a directory
        fs.mkdirSync(destination);
    }
    else if (!wasFile && shouldBeFile) {
        // Check the file's parent directory
        var baseDir = path.dirname(destination);
        try {
            var baseStat = fs.lstatSync(baseDir);
        } catch(error) {
            log.error('Directory ' + baseDir + ' does not exist for destination ' + destination + '.');
            throw new Error('Directory ' + baseDir + ' does not exist for destination ' + destination + '.');
        }
    }

    // Create some sort of stat Object
    return {
        "path":        destination,
        "isFile":      shouldBeFile,
