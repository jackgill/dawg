...
function serve(source, options) {
    ...
    function updateChapters() {
        logger.log('verbose', 'Gathering chapters from ' + source);
    ...
    function renderChapters() {
        ...
        logger.log('verbose', 'Rerendering ' + chapters.length + ' chapters');
    ...
    logger.log('dawg', 'dawg listening on ' + options.host + ':' + options.port);
}
...
function convert(source, destination, options) {
    ...
    function converter() {
        logger.log('dawg', 'Converting ' + source + ' to ' + destination);
...
function write(chapters, destination, options) {
    ...
    if (type.isFile() && chapters.length > 1) {
        logger.log('error', 'Cannot write more than one chapter to a single file');
        return;
    }

    // Render each chapter
    logger.log('verbose', 'Rendering ' + chapters.length + ' chapters');
    ...
    logger.log('verbose', 'Writing ' + chapters.length + ' chapters to ' + destination);
...
function createDestination(destination, unlink) {
    logger.log('conv', 'Creating destination ' + destination);
